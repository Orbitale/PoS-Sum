...
fn reset_sales_inner(state: &DbState) -> Result<(), String> {
    let conn = state
        .conn
        .lock()
    ...
    .map_err(|e| format!("Failed to reset sales: {e}"))?;
...
#[tauri::command]
pub fn reset_sales(state: State<'_, DbState>) -> Result<(), String> {
    reset_sales_inner(&state)
}
